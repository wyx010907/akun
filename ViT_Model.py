...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        ...
        # flatten(2)从下标为2开始后展平==>(B, C, HW) (16, 3, 196)
        # transpose(1, 2)下标为1 2位置互换==>(B, HW, C) (16, 196, 3)
...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x):
        ...
        if self.dist_token is None:
            x = torch.cat((cls_token, x), dim=1)  # [B, 196, 768]
        else:
            x = torch.cat((cls_token, self.dist_token.expend(x.shape[0], -1, -1), x), dim=1)
